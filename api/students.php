...
$storeFile = $baseDir . DIRECTORY_SEPARATOR . 'students.json';
$profilesDir = $baseDir . DIRECTORY_SEPARATOR . 'students';
...
if ($method === 'GET') {
    if (isset($_GET['id'])) {
        ...
        http_response_code(404); echo json_encode(['error'=>'Student not found']); exit;
    }
...
if ($method === 'POST') {
    ...
    $rec = [
        'id' => $next,
        'username' => $data['username'] ?? ('student'.$next),
...
if ($method === 'PUT') {
    ...
    foreach ($list as &$item) {
        if (strval($item['id']) === strval($data['id'])) {
            foreach (['username','full_name','email','level','timezone'] as $k) { if (array_key_exists($k,$data)) $item[$k]=$data[$k]; }
            $updated = $item; break;
        }
    }
    if (!$updated) { http_response_code(404); echo json_encode(['error'=>'Student not found']); exit; }
...
if ($method === 'DELETE') {
    ...
    if (count($list) === $before) { http_response_code(404); echo json_encode(['error'=>'Student not found']); exit; }
    save_store($storeFile, $list);
